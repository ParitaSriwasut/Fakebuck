...
const getTargetUserStatusWithAuthUser = async (targetUserId, authUserId) => {
  ...
  //if not yourself then find relationship
  //userId 2 , authUser id  : req.user.id (from authenticatedMiddleware)
...
const getTargetUserFriends = async (targetUserId) => {
  // STATUS = 'ACCEPTED AND (requesterId = targetUserId or receiverId= targetUserId)'
...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    // fs is lib for manage node js which in this case is Method unlink
    if (req.files.profileImage) {
...
exports.getUserById = async (req, res, next) => {
  try {
    ...
    const userId = +req.params.userId; // Profile Owner
    ...
    //if user have value then find thierFriend
